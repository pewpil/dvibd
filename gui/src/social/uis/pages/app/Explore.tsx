...
import styles from '~/social/styles/pages/app/Explore.module.css';
...
const tags: { tag: string; color: Color; posts: string }[] = [
  ...
  { tag: '#solidjs', color: 'teal', posts: '2.1k' },
  { tag: '#dvibd', color: 'coral', posts: '430' },
];
  ...
  return (
    <div class={styles.page}>
      <header class={`container ${styles.head}`}>
        <h1 class={styles.title}>Explore</h1>
        <p class={styles.lead}>See what the wider dvibd community is talking about.</p>
      </header>

      <div class={`container ${styles.layout}`}>
        <section class={styles.tags}>
          <h2 class={styles.h2}>Trending tags</h2>
          <For each={tags}>
            {(t) => (
